...
Friend Class StreamHelper
    ...
    Public Shared Function BeginReadStreamToEnd(ByVal stream As Stream, ByVal buffer As Byte(), ByVal offset As Integer, _
                                                ByVal count As Integer, ByVal callback As AsyncCallback, _
        ...
        Dim onReadCallback As New AsyncCallback(AddressOf StreamHelper.OnRead)
        stream.BeginRead(tempBuffer, 0, count, onReadCallback, asyncState)
        Dim asyncResult As IAsyncResult = result
        Return asyncResult
    End Function

    Public Shared Function EndReadStreamToEnd(ByVal ar As IAsyncResult) As Integer
    ...
    Private Shared Sub OnRead(ByVal ar As IAsyncResult)
        If (Not ar Is Nothing) Then
            ...
            If (Not state Is Nothing) Then
                ...
                Else
                    ...
                    Dim onReadCallback As New AsyncCallback(AddressOf StreamHelper.OnRead)
                    state.Stream.BeginRead(state.Result.TempBuffer, 0, _
                                           CInt(state.Result.Result.Length) - state.Result.Length, onReadCallback, state)
                End If
            End If
        End If
    End Sub
